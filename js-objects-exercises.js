...
function getFullNames(customers){
        ...
        for (var i = 0; i < customers.length; i = i + 1){
          var fullName = customers[i].first + ' ' + customers[i].last
            wholeName.push(fullName);
          }
        return wholeName;
}
...
var createListOfObjects =function (arrOfStr){
      ...
      //going to need to split them and put them into a new array.
      //split and push?
            var ninjaObj = {};
      var ninjaNames = arrOfStr[i];
          ninjaNames = ninjaNames.split(' ');
          ninjaObj.firstName = ninjaNames[0];
          ninjaObj.lastName = ninjaNames[1];
          newNinjas.push(ninjaObj)
...
var generateDog = function(dogName) {
  var dogList = {}
  dogList.name = dogName
  dogList.legs = 4
  dogList.weight = "200lb"
  dogList.color ="white"
  dogList.speak = "str"

      return dogList
}
 var changeR = function (str){
    var newSpeak = [];
      for(var i = 0; i < str.length; i ++ ){
          var newWord = str[i];
          var newSent = newWord.split("")
          newSent[0] = newSent[0] = "r"
      }
          var speaking = newSent.join("")
          return newSpeak
 }
...
// var dogSpeak = function(str){
//     var scoobySnack = str.split(" ");
//     var shaggy = [];
//     var shaggySnacks = [];
//     for ( var i = 0; i < scoobySnack.length; i ++){
//       var scoobyDoo = scoobySnack[i];
//       var scoobs = scoobyDoo.split("")
//       scoobs[0] = scoobs[0] + "r"
//       var speak = scoobs.join("")
//
//       shaggy.push(speak)
//
//     }
//     var shaggySnacks = shaggy.join("")
//
//       return shaggySnacks
// }
...
var pluck = function (arrOfObjects, propName ) {
          ...
          for(var i = 0; i < arrOfObjects.length; i ++){
             var newObject = arrOfObjects[i];
             var valueProp = newObject[propName];
             newArray.push(valueProp)
          }
            return newArray
...
var getsCounts = function(strOfText){
        var wordCountObject = {}
...
console.log( reversed['2b'] === 'apartment_no' )
